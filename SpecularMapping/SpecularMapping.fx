...
// Pass 0
//--------------------------------------------------------------//
string SpecularMapping_Pass_0_Model : ModelData = "..\\SpecularMapping\\sphere.x";
...
<
   string ResourceName = "..\\SpecularMapping\\fieldstone_SM.tga";
>;
...
float4 SpecularMapping_Pass_0_Pixel_Shader_ps_main(PS_INPUT Input) : COLOR
{
   ...
   float4 specularMap = tex2D(SpecularSampler, Input.mUV);
   float3 diffuse = albedo * gLightColor * saturate(Input.mDiffuse);
   ...
   if ( diffuse.x > 0 )
   {
      ...
      specular = specularMap * gLightColor * specular;
   }
