...
D3DXVECTOR4				gSurfaceColor(0.0f, 1.0f, 0.0f, 1.0f);
...
void RenderFrame()
{
	D3DCOLOR bgColour = 0xFF000000;	// 배경색상 - 파랑
...
void RenderScene()
{
	...
	// 1.Inverse나 Transpose 먼저 하고 Rotation 하는 경우
	// Inverse Matrix가 매우 큰 값으로 고정됨. 바뀌지 않음.

	// 2.Rotation 함수는 중복하여 동작하지 않는다.
	// 맨 마지막 Rotation을 기준으로 동작하게 되는데, 이러한 동작 원리를 이해할 필요가 있다..
	...
	std::string FilePath = "";
	FilePath += std::to_string(matInvWorld._11) + " ";
	FilePath += std::to_string(matInvWorld._12) + " ";
	FilePath += std::to_string(matInvWorld._13) + " ";
	FilePath += std::to_string(matInvWorld._14) + " \n";

	FilePath += std::to_string(matInvWorld._21) + " ";
	FilePath += std::to_string(matInvWorld._22) + " ";
	FilePath += std::to_string(matInvWorld._23) + " ";
	FilePath += std::to_string(matInvWorld._24) + " \n";

	FilePath += std::to_string(matInvWorld._31) + " ";
	FilePath += std::to_string(matInvWorld._32) + " ";
	FilePath += std::to_string(matInvWorld._33) + " ";
	FilePath += std::to_string(matInvWorld._34) + " \n";

	FilePath += std::to_string(matInvWorld._41) + " ";
	FilePath += std::to_string(matInvWorld._42) + " ";
	FilePath += std::to_string(matInvWorld._43) + " ";
	FilePath += std::to_string(matInvWorld._44) + " \n";

	gpFont->DrawText(NULL, FilePath.c_str(), -1, &rct, 0, fontColor);
