...
float4 EnvironmentMapping_Pass_0_Pixel_Shader_ps_main(PS_INPUT Input) : COLOR
{
   ...
   //Input Normal is not Normalized..?
   //same result : 
   //tangentNormal = normalize(Input.mNormal * 2 - 1);
   ...
   float3 viewReflect = reflect(viewDir, Input.mNormal);
   float3 environment = texCUBE(EnvironmentSampler, viewReflect).rgb;
   return float4(ambient + specular + diffuse + environment * 0.5, 1);
}
//--------------------------------------------------------------//
