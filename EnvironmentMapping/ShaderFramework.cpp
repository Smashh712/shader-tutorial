...
// 쉐이더
LPD3DXEFFECT			gpEnviromentMappingShader = NULL;
...
void RenderScene()
{
	...
	// gWorldCameraPosition.x += gRotationY;
	...
	gpEnviromentMappingShader->SetMatrix("gWorldMatrix", &matWorld);
	gpEnviromentMappingShader->SetMatrix("gWorldViewProjectionMatrix", &matWorldViewProjection);

	gpEnviromentMappingShader->SetVector("gWorldLightPosition", &gWorldLightPosition);
	gpEnviromentMappingShader->SetVector("gWorldCameraPosition", &gWorldCameraPosition);

	gpEnviromentMappingShader->SetVector("gLightColor", &gLightColor);

	gpEnviromentMappingShader->SetTexture("DiffuseMap_Tex", gpStoneDM);
	gpEnviromentMappingShader->SetTexture("SpecularMap_Tex", gpStoneSM);
	gpEnviromentMappingShader->SetTexture("NormalMap_Tex", gpStoneNM);

	gpEnviromentMappingShader->SetTexture("EnvironmentMap_Tex", gpSnowENV);

	UINT numPasses;
	gpEnviromentMappingShader->Begin(&numPasses, NULL);
	{
		for (UINT i = 0; i < numPasses; ++i) {
			gpEnviromentMappingShader->BeginPass(i);
			{
				gpTeapot->DrawSubset(0);
			}
			gpEnviromentMappingShader->EndPass();
		}
	}
	gpEnviromentMappingShader->End();
}
...
bool LoadAssets()
{
	...
	// 쉐이더 로딩
	gpEnviromentMappingShader = LoadShader("EnvironmentMapping.fx");
	if (!gpEnviromentMappingShader)
	{
		return false;
	}
...
void Cleanup()
{
	...
	if (gpEnviromentMappingShader)
	{
		gpEnviromentMappingShader->Release();
		gpEnviromentMappingShader = NULL;
	}
